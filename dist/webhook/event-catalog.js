...
export function parseWebhookRuntimeCatalog(result) {
...
export function resolveWebhookEventSelection(value, catalog, options = {}) {
    if (!value)
        throw new Error("Missing required option: --events");
    const tokens = value
        .split(",")
    ...
    for (const token of tokens) {
        ...
        if (token === "all") {
            ...
            if (missing.length > 0)
                missingByToken.set(token, missing);
        }
            ...
            expansion = [token];
            if (!runtimeEvents.has(token))
                missingByToken.set(token, [token]);
        }
        if (presets.includes(token))
            presetExpansions[token] = dedupe(expansion);
    ...
    if (missingByToken.size > 0) {
        const details = [...missingByToken.entries()]
            ...
            .join("; ");
        throw new Error(`Webhook event selection is not supported by the runtime GET /webhook/events catalog. Missing events: ${details}. No events were written.`);
    }
    const warnings = [];
    if (tokens.includes("core"))
        warnings.push(CORE_WARNING);
...
export function describeWebhookPresets(catalog) {
    return Object.fromEntries(WEBHOOK_PRESET_NAMES.map((name) => [name, resolveWebhookEventSelection(name, catalog).resolvedEvents]));
}
...
function parseRuntimeAliases(value) {
    if (isRecord(value)) {
        return Object.fromEntries(Object.entries(value)
            .map(([name, events]) => [name.toLowerCase(), normalizeAliasEvents(events)])
    ...
    if (Array.isArray(value)) {
        ...
        for (const item of value) {
            ...
            if (events.length > 0)
                aliases[item.name.toLowerCase()] = events;
        }
        return aliases;
    }
    return {};
}
function normalizeAliasEvents(value) {
    const events = Array.isArray(value)
        ? value.filter((event) => typeof event === "string")
        : typeof value === "string"
            ? value.split(",")
            : [];
